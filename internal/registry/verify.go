...
func verifySums(keys []gpgKey, sums, sig []byte, refreshed []string, allowExpired bool) (keyID string, expired bool, err error) {
	...
	if !allowExpired || !errors.Is(err, pgperrors.ErrKeyExpired) {
...
func verifyAtSigningTime(keyring openpgp.EntityList, sums, sig []byte) (*openpgp.Entity, error) {
	signedAt, err := sigCreationTime(sig)
	if err != nil {
	...
	if signedAt.After(now) {
		return nil, fmt.Errorf("%w: signature claims a creation time in the future (%s)",
			ErrSignature, signedAt.UTC().Format(time.RFC3339))
	}

	cfg := &packet.Config{Time: func() time.Time { return signedAt }}
	signer, err := openpgp.CheckDetachedSignature(keyring, bytes.NewReader(sums), bytes.NewReader(sig), cfg)
	if err != nil {
		return nil, fmt.Errorf("%w: %w", ErrSignature, err)
	}
...
func assembleKeyring(keys []gpgKey, refreshed []string) (openpgp.EntityList, error) {
	...
	for _, k := range keys {
		...
		for _, e := range ents {
			ring = append(ring, refreshEntity(e, updates))
		}
	}
	if len(ring) == 0 {
...
func refreshEntity(e *openpgp.Entity, updates openpgp.EntityList) *openpgp.Entity {
